...
/** Oxygen requirement ladder by cabin altitude. */
export function OxygenLadder(p: DiagramProps) {
  const altitude = num(p.altitude, 12000);
  const equipped = bool(p.equipped, true);

  const top = 56;
  const bottom = 244;
  const maxAlt = 16000;
  const y = (alt: number) => bottom - (alt / maxAlt) * (bottom - top);

  const required = altitude > 10000;
  const prohibited = equipped ? altitude > 13000 : altitude > 12000;
  const limited = equipped ? altitude > 10000 && altitude <= 13000 : altitude > 10000 && altitude <= 12000;

  return (
    <Diagram title="Oxygen requirements by cabin altitude">
      {/* Bands */}
      <rect x={70} y={top} width={250} height={y(13000) - top} fill="color-mix(in srgb, var(--color-nogo) 12%, transparent)" />
      <rect x={70} y={y(13000)} width={250} height={y(10000) - y(13000)} fill="color-mix(in srgb, var(--color-caution) 12%, transparent)" />
      <rect x={70} y={y(10000)} width={250} height={bottom - y(10000)} fill="color-mix(in srgb, var(--color-go) 10%, transparent)" />

      {[0, 10000, 13000, 16000].map((a) => (
        <g key={a}>
          <line x1={64} y1={y(a)} x2={320} y2={y(a)} stroke={MUTED} strokeWidth={1} strokeDasharray="3 3" />
          <text x={58} y={y(a) + 4} textAnchor="end" fontSize={9.5} fontWeight={700} fill={MUTED} className="tabular">
            {a.toLocaleString()}
          </text>
        </g>
      ))}

      <text x={196} y={y(14500)} textAnchor="middle" fontSize={10} fontWeight={800} fill={NOGO}>
        Above 13,000 — prohibited without oxygen
      </text>
      <text x={196} y={y(11500)} textAnchor="middle" fontSize={10} fontWeight={800} fill={CAUTION}>
        10,000–13,000 — 3 hours maximum
      </text>
      <text x={196} y={y(5000)} textAnchor="middle" fontSize={10} fontWeight={800} fill={GO}>
        Below 10,000 — no requirement
      </text>

      {/* Current altitude marker */}
      <line x1={70} y1={y(altitude)} x2={330} y2={y(altitude)} stroke={NAVY} strokeWidth={2.4} />
      <circle cx={330} cy={y(altitude)} r={5} fill={NAVY} />

      <g transform="translate(410 100)">
        <rect x={-72} y={-30} width={144} height={56} rx={11} fill="var(--color-surface-2)" />
        <text x={0} y={-12} textAnchor="middle" fontSize={9.5} fontWeight={750} fill={MUTED}>
          CABIN ALTITUDE
        </text>
        <text x={0} y={12} textAnchor="middle" fontSize={17} fontWeight={800} fill={NAVY} className="tabular">
      ...
      <g transform="translate(410 186)">
        <rect
          ...
          height={56}
          rx={11}
          fill={`color-mix(in srgb, ${prohibited ? NOGO : required ? CAUTION : GO} 15%, transparent)`}
          stroke={prohibited ? NOGO : required ? CAUTION : GO}
          strokeWidth={1.8}
        />
        <text
          x={0}
          ...
          fontSize={11}
          fontWeight={800}
          fill={prohibited ? NOGO : required ? CAUTION : GO}
        >
          {prohibited ? "PROHIBITED" : required ? "OXYGEN REQUIRED" : "NO REQUIREMENT"}
        </text>
        <text x={0} y={14} textAnchor="middle" fontSize={9} fontWeight={650} fill={MUTED}>
          {limited ? "3 hours maximum" : prohibited ? "without oxygen" : "for all occupants"}
        </text>
      </g>

      <text x={250} y={278} textAnchor="middle" fontSize={9.5} fontWeight={650} fill={MUTED}>
        Unpressurized aircraft {equipped ? "with" : "without"} an oxygen system
      </text>
    </Diagram>
  );
}
