"use client";
import { ArrowRight, Check, Star, Target } from "lucide-react";
...
export function FlightDebrief({ lesson, score, xp, before, nextId }: {
  ...
  return <section className="flight-debrief" aria-labelledby="debrief-title">
    ...
    <div className="debrief-badge"><LessonToken icon={lesson.mapIcon} size={106} state={score===1?"perfect":"completed"} celebrate={score>=.8}/></div>
    ...
    <div className="debrief-totals"><span><Star size={20}/><strong>+{shown}</strong> XP earned</span><span><Check size={20}/><strong>{Math.round(score*100)}%</strong> first try</span></div>
